    ...
    SubShader
    {
        CGPROGRAM
            ...
            void surf (Input IN, inout SurfaceOutput o) //shader function
            {
                ...
                o.Emission = frac(IN.worldPos.y*10 * _RimPower) > 0.4 ? //funkcja frac zwraca wartości po przecinku
                tex2D(_MainTexture, IN.uv_MainTexture).rgb * float3(0,1,0): tex2D(_MainTexture, IN.uv_MainTexture).rgb * float3(1,0,0);
            }   
